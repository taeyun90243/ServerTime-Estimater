...
- **알고리즘(신규 함수 0개)**: 기존 `Invoke-AdaptiveMultiSample`을 `-TargetWindowMs 2500 -MinEdgeCount 1 -MaxTotalMs 5000 -DefaultTimeoutMs 2000`로 1회 호출. `MinEdgeCount=1`이라 연장 루프 비활성화(5초 내 종료). edge 1~3개 → ±100~300ms, 0개(frozen) → ±500ms.
  - **`-DefaultTimeoutMs 2000` 필수**: RTT probe 단계의 데드라인 reserve가 `DefaultTimeoutMs`다. fast의 5초 budget 안에서 probe 실행 + 적응형 timeout ≤2초로 cap 유지.
